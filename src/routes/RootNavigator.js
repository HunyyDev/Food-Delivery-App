...
import React, { createContext, useCallback, useEffect, useLayoutEffect, useMemo, useReducer, useState } from 'react';
...
const RootNavigator = () => {
  const [state, dispatch] = useReducer(setStatus, { isLoggedIn: false, isLoading: true });
  useEffect(() => {
    const getData = async () => {
  ...
  if (!isOnline) {
    return (
      <NoInternetScreen state={isOnline} setter={tricky} />
    )
  }
